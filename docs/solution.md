...
2. **The Mamba component → painfully slow training and a finicky environment.** Mamba relies on dedicated CUDA kernels such as `mamba_ssm` and `causal_conv1d`, which have poor compatibility with Kaggle's new **Blackwell GPUs**. Kaggle's 3-hour limit allows training only about 20 million tokens, whereas the 1st-place team used their own workstation to train **over a billion tokens across hundreds of hours** — a substantial compute barrier.
